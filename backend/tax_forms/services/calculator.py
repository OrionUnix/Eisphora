"""
Calcul des plus-values crypto – Art. 150 VH bis du CGI (France)
================================================================
Corrections apportées par rapport à la version originale :

1. [FISCAL] Numérateur de la fraction = prix de CESSION BRUT (pas net).
   Source : BOFIP BOI-RPPM-PVBMC-20-10 §130.

2. [FISCAL] La PV nette = Prix_cession_brut - Prix_acq_fractionné - Frais_cession.
   Les frais se déduisent APRÈS le calcul de la fraction, pas avant.

3. [PERF]   Pré-chargement groupé des prix historiques (batch prefetch)
   avant la boucle principale, pour éviter N appels API séquentiels.

4. [ROBUSTESSE] Gestion des edge-cases (portfolio vide, qty nulle, etc.)

5. [FISCAL] Nouveau type 'transfert_interne' pour les mouvements comptables
   internes Coinbase (Retail Staking/Unstaking Transfer) :
   ces opérations par paires (+qty/-qty) n'ont aucune réalité fiscale,
   ni PTA modifié, ni événement imposable, ni changement de portefeuille.
"""
...
def calculate_french_taxes(
    ...
    Règles appliquées
    -----------------
    • Seules les cessions contre une vraie monnaie fiduciaire sont imposables.
    • Échanges crypto ↔ crypto (y compris stablecoins) : sursis d'imposition.
    • Les moins-values s'imputent sur les plus-values de la même année fiscale.
    • Méthode de valorisation : Prix Moyen Pondéré Global (PMP global).
    • Formule légale BOFIP §130 :
        Prix_acq_fractionné = PTA × (Prix_cession_BRUT / Valeur_globale_portefeuille)
        Plus_value_nette    = Prix_cession_BRUT − Prix_acq_fractionné − Frais_cession
    ...
    # ------------------------------------------------------------------
    # 2. Pré-chargement groupé des prix (optimisation performance)
    # ------------------------------------------------------------------
    price_needs = _collect_price_needs(transactions)
    price_cache: Dict[Tuple[str, str], float] = _batch_fetch_prices(price_needs)
    ...
    total_prix_cession_brut: float = 0.0  # Somme des cessions brutes taxables
    global_plus_value: float = 0.0        # Bilan net PV − MV de l'année
    ...
    for idx, tx in enumerate(transactions):
        ...
        # --- Lecture et validation des données de la transaction ---
        try:
        ...
        # ==============================================================
            ...
            # B. Valeur Globale du Portefeuille (VGP) au moment de la cession
            valeur_globale = 0.0
            for asset, asset_qty in list(portfolio.items()):
                if asset_qty <= _DUST_THRESHOLD:
                    continue
                if asset == crypto:
                    p = unit_price  # on utilise le prix de cession, déjà connu
                else:
                    p = _get_cached_price(asset, date_str, price_cache)
                valeur_globale += asset_qty * p

            # Sécurité : la VGP ne peut pas être inférieure à la valeur brute
            # de la crypto cédée (elle fait forcément partie du portefeuille).
            valeur_globale = max(valeur_globale, prix_cession_brut, 1e-6)
        ...
        # ==============================================================
        # DÉPÔT / TRANSFERT ENTRANT (entre wallets personnels)
        # ==============================================================
        elif op in ('depot', 'deposit'):
            # Simple mouvement entre wallets : le PTA ne change PAS.
            portfolio[crypto] = portfolio.get(crypto, 0.0) + qty
            tx['remaining_quantity'] = portfolio[crypto]
